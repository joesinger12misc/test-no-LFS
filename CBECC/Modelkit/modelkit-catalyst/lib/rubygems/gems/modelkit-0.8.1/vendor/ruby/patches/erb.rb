...
# NOTE: This is a patched version of the ERB 2.2.0 standard library file. It includes
# nearly all of the bug fixes and performance improvements from ERB 2.2.0, but
# preserves compatiblity with old versions of Ruby back to Ruby 2.0 with a handful
# of surgical changes. This patched version also preserves the original signature
# of the constructor method: `ERB.new(str, safe_level, trim_mode, eoutvar)` This is
# necessary to keep compatibility with the GLI gem that also uses ERB.
...
#
class ERB
  Revision = '$Date:: 2018-11-12 09:37:15 +0900#$' # :nodoc: #'
...
class ERB
  ...
  class Compiler # :nodoc:
    ...
    def compile_content(stag, out)
      # Strip any final end-of-line comment. Otherwise the comment can unintentionally
      # cancel out any following ERB block on the same line. For example:
      #   <% x = 1  # comment %><% y = 2 %>
      # The y = 2 block is never evaluated unless the comment in the first block is
      # removed first. It used to compile into this:
      #   ; x = 1  # comment ;  y = 2
      # NOTE: The regex is not perfect. A quote in the comment will break it.
      content.sub!(/#[^'"\n]*\z/, "")
    ...
    def warn_invalid_trim_mode(mode, uplevel: 1)
