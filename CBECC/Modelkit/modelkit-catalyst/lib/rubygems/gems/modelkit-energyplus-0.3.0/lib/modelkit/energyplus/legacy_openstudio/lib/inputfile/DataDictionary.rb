...
require("inputfile/FieldDefinition")
require("inputfile/ClassDefinition")
          ...
          while (line = idd.gets)
            ...
            if (line.strip.empty?)
              ...
              while (line = idd.gets)
                ...
                if (i = line.index("\\memo"))
                  # Object description line
                  class_def.description += line[(i + 6)..(line.length)].strip
                elsif (i = line.index("\\unique-object"))
                ...
                elsif (i = line.index("\\autosizable"))
                  if (defined? field_def)
                    field_def.autosizable = true
                  end
