...
module OpenStudio
  ...
  class InputObject
    ...
    def initialize(class_name, fields = nil, context = nil)
      ...
      if (@class_definition = $data_dictionary.get_class_def(class_name))
    ...
    def InputObject.new_from_idf(idf_string)
      ...
      for line in lines
        ...
        if (semicolon_line or comma_line)
          ...
          if (semicolon_line)
            # Replace the semicolon with a comma; add extra space to allow entering for loop below after pop
            line_no_comment = line_no_comment[0...semicolon_line] + ', '
          end
          ...
          for n in 0...line_fields.length
            ...
            if (field.empty?)
              ...
              if (a.nil?)
                # this will happen if we have a semicolon line
                a = line.index(';')
              end
              line.insert(a, token)
      ...
      if (class_def = $data_dictionary.get_class_def(fields[0]))
        ...
        puts "Error:  Unidentified EnergyPlus class " + fields[0].to_s + ".\n"
        ...
        puts "The object could not be parsed.  You might have old version of EnergyPlus or an old IDD--try transitioning your file to the latest version.\n\n"
        return(nil)
      end
