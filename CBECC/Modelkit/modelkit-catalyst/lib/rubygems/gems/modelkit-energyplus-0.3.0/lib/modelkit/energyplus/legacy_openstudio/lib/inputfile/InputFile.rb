...
# needs a copy method
  ...
  class InputFile
    ...
    def InputFile.open(data_dictionary, path, update_progress = nil)
      $data_dictionary = data_dictionary  # kludge because InputObject looks for Plugin.data_dictionary
...
# could put all File.exist? checking at the read_file, write_level
    ...
    def find_object_by_class_and_name(class_name, object_name)
      return(@objects.find { |object| object.is_class_name?(class_name) and object.name.upcase == object_name.upcase })
    end
  ...
  private
    ...
    # NOTE: This does not parse multiple objects on the same line.
    def read_file(path)
      ...
      while (line = file.gets)
        ...
        if (semicolon_line or comma_line or blank_line)
          ...
          if (semicolon_line and comma_line and comma_line > semicolon_line)
            # New object starts on the same line after a semicolon ending the previous object.
            # Multiple objects on the same line is not handled, but the simple case where
            # a new objects _starts_ but does not finish at the end of another object is
            # attempted. In certain cases a trailing newline can get truncated.

            puts "Warning: New object starts on same line after semicolon"

            buffer += line[0..semicolon_line]
            ...
            buffer = line[semicolon_line + 1..-1].lstrip  # Restart buffer with rest of line
            inside_object = true
            end_object = false

          elsif (semicolon_line)
            buffer += line
            inside_object = true
