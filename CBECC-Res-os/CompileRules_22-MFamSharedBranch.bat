echo off
  echo --------------------------------------------
  echo Compiling 2022 ruleset...
BEMCompiler22r.exe --sharedPath1="../../branches/RulesetDev_MFamRestruct/Rulesets/shared/" --bemBaseTxt="../../branches/RulesetDev_MFamRestruct/Rulesets/CA Res/CAR13 BEMBase.txt" --bemEnumsTxt="../../branches/RulesetDev_MFamRestruct/Rulesets/CA Res/CAR22 BEMEnums.txt" --bemBaseBin="Data/Rulesets/CA Res 2022/CAR22 BEMBase.bin" --rulesTxt="../../branches/RulesetDev_MFamRestruct/Rulesets/CA Res/Rules/Rules-2022.txt" --rulesBin="Data/Rulesets/CA Res 2022.bin" --rulesLog="../../branches/RulesetDev_MFamRestruct/Rulesets/CA Res/Rules/Rules-2022 Log.out" --compileDM --compileRules
echo BEMCompiler22r.exe returned (%ERRORLEVEL%) for CA Res 2022
if %ERRORLEVEL%==0 goto :copyfiles2
goto :error2
:copyfiles2
copy "..\..\branches\RulesetDev_MFamRestruct\Rulesets\CA Res\CAR22 Screens.txt"  "Data\Rulesets\CA Res 2022\*.*"
copy "..\..\branches\RulesetDev_MFamRestruct\Rulesets\CA Res\CAR13 ToolTips.txt" "Data\Rulesets\CA Res 2022\CAR22 ToolTips.txt"
copy "..\..\branches\RulesetDev_MFamRestruct\Rulesets\CA Res\RTF\*.*" "Data\Rulesets\CA Res 2022\RTF\*.*"
copy "..\..\branches\RulesetDev_MFamRestruct\Rulesets\shared\Screens*.txt" "Data\Rulesets\CA Res 2022\*.*"
copy "..\..\branches\RulesetDev_MFamRestruct\Rulesets\CA Res\DHWDU2.txt" "CSE\*.*"
goto :done2
:error2
  echo --------------------------------------------
  echo Rule compilation errors occurred.
  echo See log file for details:  RulesetDev/Rulesets/CA Res/Rules/Rules-2022 Log.out
  echo --------------------------------------------
  pause
:done2